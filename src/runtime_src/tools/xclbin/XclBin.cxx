...
XclBin::printHeader() {
  ...
  Section* pSection = findSection(BUILD_METADATA);
  if (pSection == nullptr) {
    std::cout << "No build metadata section present.\n";
  } else {
    std::cout << "DSA Information\n";
    boost::property_tree::ptree pt;
    pSection->getPayload(pt);
    // Here we read all out as a string
    std::string sDsaGenProductName = pt.get<std::string>("build_metadata.dsa.generated_by.name", "");
    std::string sDsaGenVersion = pt.get<std::string>("build_metadata.dsa.generated_by.version", "");
    std::string sDsaGenCl = pt.get<std::string>("build_metadata.dsa.generated_by.cl", "");
    std::string sDsaGenTimeStamp = pt.get<std::string>("build_metadata.dsa.generated_by.time_stamp", "");
    std::cout << "DSA generated by : " << sDsaGenProductName << " " 
      << sDsaGenVersion << "(" << sDsaGenCl << ") " << sDsaGenTimeStamp << "\n";
  }
